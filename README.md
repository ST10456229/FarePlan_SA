...
FarePlan SA includes unit tests covering key areas of the application's business logic.

## Unit Tests

Two test suites are currently included.

### `BudgetAlertHelperTest.kt`

Tests the application's budget monitoring and alert logic.
...
* `50%`
* `100%`
* Values exceeding the budget are capped at `100%`
...
Tests the application's currency conversion functionality.
...
* Unsupported currencies returning `null`
...
Test results are generated by Gradle after the test suite completes.
...
Every push to the `master` branch automatically triggers the CI workflow.
...
1. Set up **JDK 17**
2. Restore Firebase configuration from an encrypted GitHub repository secret
3. Run all unit tests
4. Build the debug APK
5. Report the test and build results
...
FarePlan SA integrates Firebase services and external APIs to provide authentication, cloud storage, notifications, currency conversion, and travel search functionality.
...
The database is organised around users and their associated trips.
...
| Collection                 | Purpose                                        |
| -------------------------- | ---------------------------------------------- |
| `users`                    | Stores user profile and preference information |
| `users/{userId}/trips`     | Stores the user's trips and budget information |
| `trips/{tripId}/itinerary` | Stores itinerary items                         |
| `trips/{tripId}/expenses`  | Stores trip expenses                           |

---

# 🌐 Localization

FarePlan SA supports three languages using Android's resource system.

| Language       | Resource Folder | Coverage |
| -------------- | --------------- | -------: |
| 🇬🇧 English   | `values/`       |     100% |
| 🇿🇦 isiZulu   | `values-zu/`    |     100% |
| 🇿🇦 Afrikaans | `values-af/`    |     100% |

Users can change the application language from the **Settings** screen.

The application restarts to apply the selected language.

## 🇿🇦 South African Formatting

FarePlan SA uses South African-friendly date and currency formatting.
...
Screenshots of the application's main screens are stored in:
...
`https://github.com/ST10456229/FarePlan_SA`
...
| Item                      | Action                                                                        |
| ------------------------- | ----------------------------------------------------------------------------- |
| 🎥 Demo Video             | Replace `https://youtu.be/YOUR_VIDEO_ID` with the actual unlisted YouTube URL |
| 📸 Screenshots            | Ensure all referenced screenshots exist                                       |
| 🏗️ Architecture Diagram  | Add `docs/architecture.png` or remove the reference                           |
| 🤖 AI Documentation       | Add `AI_USAGE.md` if detailed AI usage documentation is required              |
| 🔐 Firebase Configuration | Confirm `google-services.json` is not committed                               |
| 🧪 Tests                  | Run `./gradlew test` successfully                                             |
| ⚙️ CI/CD                  | Confirm GitHub Actions workflow runs successfully                             |
...
<p align="center">
  <strong>FarePlan SA</strong><br>
  Plan smarter. Spend smarter. Travel better. ✈️
</p>
